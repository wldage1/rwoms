			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding:5px;" > 
										...
										<c:forEach var="auth3" items="${authlist3}" varStatus="vsindex3">
											<c:if test="${auth2.code == auth3.parentCode}">
												<security:authorize ifAnyGranted="${auth3.code}" >
													&nbsp;<a href="#" class="mapa" id="${auth3.code}">${auth3.name}</a>
												</security:authorize>
				...
				<tr>
					<td valign="top">
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="welcomeSkin">
							<tr>
								<th colspan="2"><msg:message code="info.prompt.statistics.info"/></th>
							</tr>
							<tr>
								<td><msg:message code="info.prompt.statistics.member.count"/>${memberCount}</td>
								<td><msg:message code="info.prompt.statistics.msg.count"/>${messageCount}</td>
							</tr>
							<tr>
								<td><msg:message code="info.prompt.statistics.msg.check.count"/>${auditingCount}</td>
								<td><msg:message code="info.prompt.statistics.msg.checked.count"/>${auditedCount}</td>
							</tr>	
							<tr>
								<td><msg:message code="info.prompt.statistics.product.count"/>${productCount}</td>
								<td><msg:message code="info.prompt.statistics.reservation.count"/>${reservationCount}</td>
							</tr>	
							<tr>
								<td><msg:message code="info.prompt.statistics.reservation.disposed.count"/>${yesDisposeReservationCount}</td>
								<td><msg:message code="info.prompt.statistics.reservation.undispose.count"/>${noDisposeReservationCount}</td>
							</tr>																					
						</table>
					</td>
				</tr>				
