...
$(document).ready(function(){
    jQuery("#roleList").jqGrid({
        ...
        url: 'grid.json',
        datatype: 'json',
        colNames: ["<msg:message code='role.name'/>", "<msg:message code='role.description'/>","<msg:message code='info.operate'/>"],
    	...
    	gridComplete: function(){
    		...
    		for(var i=0;i < ids.length;i++){
    		 ...
    		 <%-- <c:if test="${!empty role_source}">
    		    <security:authorize ifAnyGranted="${role_source.code}">	
    			content += "<a href='javascript:void(0);' title='${role_source.name}' id='" + id + "' class='shortcut_${role_source.index}' title='${role_source.name}'>";
    			content += "<img src='${base}/${common}/${style}/images/icon/auth.png' weight='18' height='18' border='0' align='absmiddle'/>${role_source.name}";
    			content += "</a>";
    			content += space;
    		    </security:authorize>
    		    </c:if> --%>
    	...
    	loadComplete:function(){
    	    ...
    	    <%-- $(".shortcut_${role_source.index}").click(function(){
    	    	var rowid = $(this).attr("id");
    			window.location.href = "${base}${role_source.controller}?c=${role_source.code}&id="+rowid;
    	    }); --%>
    	},
    ...
    <security:authorize ifAnyGranted="${role_delete.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${role_delete.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/delete.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${role_delete.name}");
	$("#t_roleList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));
    $("#${role_delete.index}","#t_roleList").click(function(){
		...
		if (ids != null){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
				var url = "${base}${role_delete.controller}.json?c=${role_delete.code}&ids="+ids+"&name="+encodeURIComponent(encodeURIComponent(names))+"&prompt=name";
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
