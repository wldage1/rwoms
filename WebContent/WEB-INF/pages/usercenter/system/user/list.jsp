...
$(document).ready(function(){
    jQuery("#userList").jqGrid({
        ...
        mtype:"POST",
        postData:{c:"${user.c}",name:"${user.name}",account:"${user.account}"},
        rowNum:10,     
        page:"${user.page}",
        rowList: [10, 20, 30],
        pager: '#pagered',
    	...
    	gridComplete: function(){
    		...
    		for(var i=0;i < ids.length;i++){
    			...
    			<c:if test="${!empty user_dataauth}">
    		    <security:authorize ifAnyGranted="${user_dataauth.code}">	
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${user_dataauth.index}' ";
    			content += " title='${user_dataauth.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/auth.png' weight='18' height='18' border='0' align='absmiddle'/>${user_dataauth.name}";
    			content += "</a>";
    		    </security:authorize>
    		    </c:if> 
    ...
    <security:authorize ifAnyGranted="${user_delete.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${user_delete.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/delete.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${user_delete.name}");
	$("#t_userList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));  
    $("#${user_delete.index}","#t_userList").click(function(){
		...
		if (ids != null){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
				var url = "${base}${user_delete.controller}.json?c=${user_delete.code}&ids="+ids+"&name="+encodeURIComponent(encodeURIComponent(names))+"&prompt=name";
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
